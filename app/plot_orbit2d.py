...
# Default track duration for LEO satellites (2 orbital periods ≈ 190 min for Starlink ~550 km)
    ...
    for idx, sat in enumerate(satellites):
        ...
        try:
            ...
            ax.text(
                ...
                sat.name,
                transform=ccrs.Geodetic(),
                fontsize=4.5 if single_color is None else 7,
                color=color,
                ha="center",
                va="bottom",
                zorder=4,
...
def plot_single_orbit(
    ...
    projection: str = "robinson",
    epoch_utc: datetime = EPOCH_UTC,
    duration_minutes: int = _LEO_TRACK_MINUTES,
    output_path: str | None = None,
) -> None:
    ...
    epoch_str = epoch_utc.strftime("%Y-%m-%d %H:%M:%S UTC")
    n_orbits = duration_minutes / 95
    ax.set_title(
        f"{sat.name}  ({n_orbits:.1f} orbits shown)  [{proj_label} projection]\n{epoch_str}",
        fontsize=13,
        pad=10,
    )
    plt.tight_layout()
